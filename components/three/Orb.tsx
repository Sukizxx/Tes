...
/**
 * Cinematic monochrome orb — a slowly rotating, displaced icosahedron
 * with a fresnel rim and inner glow, plus a drifting particle field.
 * Pure transform/opacity-friendly GPU work; targets 60fps.
 */
...
const vertexShader = /* glsl */ `
  ...
  void main(){
    ...
    float n2 = snoise(normal * 3.2 - uTime * 0.12) * 0.5;
    float disp = (n + n2) * 0.16;
...
const fragmentShader = /* glsl */ `
  ...
  void main(){
    ...
    float core = smoothstep(0.0, 1.0, dot(vNormal, vec3(0.4,0.6,0.7)) * 0.5 + 0.5);
    vec3 base = mix(vec3(0.04), vec3(0.16), core);
    vec3 rim = vec3(1.0) * fresnel * 0.9;
...
function Particles() {
  ...
  return (
    <points ref={ref} geometry={geom}>
      <pointsMaterial size={0.018} color="#ffffff" transparent opacity={0.5} sizeAttenuation />
    </points>
  );
}
